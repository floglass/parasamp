...
### Create new data.frame with only positive TGAS.synth parallaxes
### !!! Row number might differ since it's a random process. Data.frame only accept same number of rows in all dimensions. !!!
# for (i in seq(iterations)){
#   if (i==1){
#     positive.indexes <- data.frame(which(TGAS.synth[[1]]>0.))
#   }
#   else{
#     positive.indexes <- cbind(positive.indexes, which(TGAS.synth[[i]]>0.))
#   }
# }
#################################################
### Error-in-variables for all the iterations ###
#################################################
### leiv.function allows to buffer for data handling. We have a data.frame (synthetic data), but need to get
### subsets of different size from that (positive data) which data.frame doesn't accept.
...
grob2 <- grobTree(textGrob(sprintf("All samples, slope: %s+/-%s, intercept: %s+/-%s", round(average.slope,3), round(error.slope,3), round(average.intercept,3),round(error.intercept,3)), x=0.01, y=0.95, hjust=0, 
                           gp=gpar(col="blue", fontsize=13)))
grob3 <- grobTree(textGrob(sprintf("All positive values, slope: %s+/-%s, intercept: %s+/-%s", round(average.pos.slope,3), round(error.pos.slope,3), round(average.pos.intercept,3), round(error.pos.intercept,3)), x=0.01, y=0.92, hjust=0,
...
p2 <-ggplot(X1, aes(x=X1[[1]], y=X1[[2]])) +
  ...
  geom_point() + 
  geom_abline(aes(color="Current sample"), intercept=fit.results[[2]][[1]], slope=fit.results[[1]][[1]], colour='black', linetype='dashed', size=1) +
  geom_abline(aes(color="All samples"), intercept=average.intercept, slope=average.slope, colour='blue', size=1) +  # averaged regression of the whole iteration dataset
  geom_abline(aes(color="Positive samples"), intercept=average.pos.intercept, slope=average.pos.slope, colour='red', size=1) +
  geom_vline(xintercept=0, colour='black', linetype='dashed') + 
  #geom_hline(yintercept=fit.results[[1]][[2]], colour='black') +
  xlab("synthetic TGAS parallaxes [mas]") + ylab("synthetic seismic parallaxes [mas]") + ggtitle("Synthetic data (showing one)") + 
